...
class FileTreeBase
	...
	def getClientByWS(ws)
		if (@clients[ws])
			return(@clients[ws])
		elsif
			puts "Invalid client with socket: #{ws}, called by " + caller_locations(1,1)[0].label.to_s
			return FALSE
		end
	end
...
class FileTree < FileTreeBase
	def procMsg(client, jsonMsg)
		puts "Asked to process a message for the FileTree from client #{client.name}"
		if (self.respond_to?("procMsg_#{jsonMsg['fileTreeCommand']}"))
			puts "Found a function handler for  #{jsonMsg['fileTreeCommand']}"
			self.send("procMsg_#{jsonMsg['fileTreeCommand']}", client, jsonMsg);
		elsif
			puts "There is no function to handle the incoming command #{jsonMsg['fileTreeCommand']}"
		end
	end
