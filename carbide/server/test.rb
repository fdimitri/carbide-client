...
class Project
	...
	def initialize(projectName)
		...
		addChat('Java')
		addChat('Coffee')
		addChat('3rd-shift-rulez')
		addChat('iMWeirD')
	end

	def start(opts = { })
		EventMachine::WebSocket.start(:host => "0.0.0.0", :port => 32760) do |ws|
	...
	def handleMessage(ws, msg)
		if (msg.is_json?)
			...
			if (jsonMsg['commandSet'] == 'document')
				puts "This message corresponds to a document"
				if (doc = getDocument(jsonMsg["document"]))
					doc.procMsg(getClient(ws), jsonMsg);
				else
					puts "handleMessage couldn't getDocument() -- document " + jsonMsg['document'] + " doesn't exist as far as we know"
				end
			elsif (jsonMsg['commandSet'] == 'chat')
				puts "This message corresponds to a chat for #{jsonMsg['chatTarget']}"
				if (chat = getChat(jsonMsg['chatTarget']))
					chat.procMsg(getClient(ws), jsonMsg)
				else
					puts "We should create a new chat since it doesn't exist"
					chat = addChat(jsonMsg['chatTarget'])
					chat.procMsg(getClient(ws), jsonMsg)
				end
			elsif (jsonMsg['commandSet'] == 'FileTree')
				@FileTree.procMsg(getClient(ws), jsonMsg)
			elsif (!jsonMsg['commandSet'] || jsonMsg['commandSet'] == 'base')
	...
	def procMsg_getChatListJSON(client = false, jsonMsg = false)
		...
		if (client != false)
			jsonMsg = jsonMsg.to_json
			sendToClient(ws, jsonMsg)
			return true
		end
		return(jsonMsg.flatten.to_json)
	end
	...
	def removeClient(ws) 
		...
		client.chats.each do |chatname, chat|
			puts "Removing client from #{chatname}"
